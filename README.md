...
>
> **Gemini 没命中？** Gemini 走 OpenAI 兼容端点时：① **不支持 `cache_control`**，设 `on` 无效，请用 `off`；
> ② 它是**隐式缓存**（仅 Gemini 2.5 系列），且要求公共前缀超过最小 token 数（Flash≈1024、Pro≈2048），
> system prompt 太短不会缓存；③ 首次请求是 miss（写入缓存），之后才命中，单独跑 1 条永远显示 0；
> ④ 命中数可能根本没经 OpenAI 兼容层透出——日志里那行 `LLM usage 字段示例` 就是给你核对的，
> 若里面确有缓存字段但我没统计到，把它发我即可补上。
...
聚焦「读卡片」：把 `cards` 表（情绪周期结构化卡片）做成可筛选、可搜索、分页的浏览界面。

```bash
pip install -r requirements.txt   # 需 streamlit / pandas
